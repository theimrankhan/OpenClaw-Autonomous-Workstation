...
install_file "${SCRIPT_DIR}/../ollama/ollama.env" \
  "${OPENCLAW_HOME}/ollama/ollama.env" 0644
...
install_file "${SCRIPT_DIR}/../services/ollama.service" \
  /etc/systemd/system/ollama.service 0644
...
# Fix for cloud-init environments where HOME is not defined
export HOME=/root

run_step "Pull default Ollama model ${OLLAMA_DEFAULT_MODEL}" \
  ollama pull "$OLLAMA_DEFAULT_MODEL"

run_step "Probe Ollama API" \
